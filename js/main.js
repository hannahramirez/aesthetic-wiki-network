...
function nodeActive(a) {
  ...
  if (groupByDirection) {
    ...
    f.push("<h2>Related aesthetic (" + size + ")</h2>");
    size > 0
      ? (f = f.concat(createList(outgoing)))
      : f.push("No outgoing links<br>");
    size = Object.size(incoming);
    f.push("<h2>Pages this aesthetic is listed on: (" + size + ")</h2>");
    size > 0
  ...
  if (f.attributes) {
    ...
    for (var attr in f.attributes) {
      ...
      if (attr == "url") {
        console.log(attr);
        h =
          '<span><strong> <a id="wiki" href="' +
          d +
          '" target="_blank">Wiki Page</a></strong></span><br/>';
      } else if (attr == "Modularity Class") {
      } else if (attr == "description") {
        h = "<span>" + d + "</span><br/>";
      } else if (attr == "image" && d != "") {
        console.log(d);    
        h = '<span><img src="' + d + '"></span><br/>';
        //h = '<span><img src="https://static.wikia.nocookie.net/aesthetics/images/8/81/Example.jpeg"></span><br/>'
  ...
  $GP.info_data.show();
  $GP.info_p.html("<h1>Connections:</h1>");
...
function toggleHide() {
  ...
  if (legend.style.display === "none") {
    ...
    mainPanel.style.backgroundColor = "rgba(255,255,255,0.3)";
    mainPanel.style.backdropFilter = "blur(20px)";
    button.innerText = "−";
  } else {
    legend.style.display = "none";
    mainPanel.style.backgroundColor = "rgba(255,255,255,0)";
    button.innerText = "+";
    mainPanel.style.backdropFilter = "blur(0px)";
  }
}
